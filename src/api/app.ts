...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("joinLobby", (code) => {
    ...
    console.log(
      "Creating new player for room " + code + " with socket id " + socket.id
    );
    game.p2 = new Player(2, socket.id, socket);
    // send data to frontend
    socket.emit("P2JoinedLobby", {
      p1char: game.p1?.char,
      p2char: game.p2.char,
      code: code,
    });
  ...
  socket.on("selectCharacter", (JSONStrPayload: any) => {
    ...
    if (gameRes) {
      ...
      if (player_num === "Player 1" && game.p1) {
        game.p1.char = char;
      }
      if (player_num === "Player 2" && game.p2) {
        game.p2.char = char;
      }
      // ! may change later
      ...
      socket
        ...
        .emit(
          "updateSelectScreen",
          { p1: game.p1?.getChar(), p2: game.p2?.getChar() },
          player_num
        );
    ...
    if (gameRes !== null) {
      ...
      if (player) {
        ...
        if (opponent) {
          ...
          const oppGameState = buildGameStateFromInstance(
            gameInstance,
            iOpponent,
            senderJSON
          );
          ...
          const senderGameState = buildGameStateFromInstance(
            gameInstance,
            senderJSON,
            iOpponent
          );
    ...
    if (gameRes) {
      ...
      if (player) {
        ...
        if (opponent) {
          ...
          console.error(
            "could not find opponent, this is an error in sendAnimatedHistory"
          );
        }
      } else {
        console.error(
          "could not find player, this is an error in sendAnimatedHistory"
        );
      }
    }
  });
        ...
        if (!p1Socket || !p2Socket) {
          ...
          if (p1 && p2) {
            ...
            if (gameInstance.currRounds > gameInstance.totalRounds) {
              ...
              console.log(
                "GAME NOT DONE ON TURN " +
                  gameInstance.currRounds +
                  " OUT OF " +
                  gameInstance.totalRounds
              );
              gameInstance.currRounds++;
              if (p1.active) {
